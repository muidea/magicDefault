...
{
    echo "buildBin..."
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o $binPath github.com/muidea/magicDefault/cmd/magicDefault
    if [ $? -ne 0 ]; then
...
function all()
{
    echo "build magicDefault docker image"
